...
ffmpeg -i "%~1" -vf "vflip,hflip,vflip,hflip" -c:a copy "%~dpn1_cw%~x1"
...
rem ffmpeg -i "%~1" -c copy -map_metadata 0 -metadata:s:v:0 rotate=90 "%~dpn1_cw%~x1"
rem ffmpeg -i "%~1" -c copy rotate=90 "%~dpn1_cw%~x1"
rem ffmpeg -i "%~1" -vf "transpose=1" "%~dpn1_cw%~x1"
